...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // UI制御用の要素もここで取得
    const fileInputContainer = document.getElementById('fileInputContainer'); // 追加
    const controlButtonsContainer = document.getElementById('controlButtonsContainer'); // 追加
    const loadingSpinnerButton = document.getElementById('loadingSpinnerButton'); // 追加

    // APP_VERSION と APP_CHANGES の定数宣言は削除されます
    ...
    async function loadVersionInfo() {
        try {
            ...
            // エラー時でも続行できるよう、ここではUIを完全にロックしない
            setUiState('ready');
            if (captureButton) captureButton.disabled = true;
            stopButton.disabled = true;
            fileInput.disabled = true;
            return null; // ロード失敗時はnullを返す
        }
    }
    ...
    window.Module.onRuntimeInitialized = async function() { // async function に変更
        const versionInfo = await loadVersionInfo(); // バージョン情報を先にロード

        if (!versionInfo) { // バージョン情報がロードできなかった場合
